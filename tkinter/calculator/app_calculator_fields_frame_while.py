...
while i < len(list_signs):
    frame = LabelFrame(text=list_descriptions[i])
    frame.pack(side='top')
    entry = Entry(frame, width=5)
    entry.grid(column=0, row=i)
    list_widgets.append(entry)
    Label(frame, text=list_signs[i]).grid(column=1, row=i)
    entry = Entry(frame, width=5)
    entry.grid(column=2, row=i)
    list_widgets.append(entry)
    Button(frame, text='=', command=list_functions[i]).grid(column=3, row=i)
