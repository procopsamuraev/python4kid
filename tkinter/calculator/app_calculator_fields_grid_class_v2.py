...
class LineCalculator:
    ...
    def __init__(self, parent: Tk, symbol_operator: str ='', operator_ : Callable = None, row: int = 0):
        self.math_sign = symbol_operator
        self.operator = operator_
        self.row = row
        self.entry1 = Entry(parent, width=5)
        self.entry1.grid(column=0, row=self.row)
        Label(parent, text=self.math_sign).grid(column=1, row=self.row)
...
for index, (sign, function) in enumerate(list_operators):
    LineCalculator(root, symbol_operator=sign, operator_= function, row=index)
