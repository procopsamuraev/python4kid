...
def get_report_error(name, qty, price, day):
    ...
    if qty_false:
        msg_warning = f"{msg_warning}\nFill up 'Qty' int, positive\n"
    if not price_valid:
        msg_warning = f"{msg_warning}\nFill up 'Price' with positive number and not more then 2 digits after dot\n"
    if not day_valid:
        msg_warning = f"{msg_warning}\nFill up 'Day' with positive number and be dividing by 0,5\n"
    return msg_warning
...
def set_total_annual():
    ...
    if (price1 or price2 or price3) == "NA":
        label_warning_global.config(text="Total Annual: Some errors with on of the product")
...
def set_total_cost():
    ...
    data_true = years.isdigit() and years > '0' and cost_annual.replace('.', '').isdigit()
