...
root = Tk()
root.title("Ex_1")
# value_sum_1 = value_sum_2 = value_result_sum = StringVar()
field_sum_1 = field_sum_2 = entry_result_sum = Entry()
list_entries = [value_sum_1, value_sum_2, field_sum_1, field_sum_2, entry_result_sum]
value_list2 = [value_sum_2]V
fields_list1 = [ field_sum_1 ]
fields_list2 = [ field_sum_2 ]
label_list = ['+']
list_functions = [ sum_numbers ]
value_result_list = [ value_result_sum ]
list_entry_result = [ entry_result_sum ]
i=0
while i < len(list_entry_result):
    value_list1[i] = StringVar()
    fields_list1[i] = Entry(root, width=5, textvariable=value_list1[i])
    fields_list1[i].grid(column=0, row=i)
    label = Label(root, text=label_list[i]).grid(column=1, row=i)

    value_list2[i] = StringVar()
    fields_list2[i] = Entry(root, width=5, textvariable=value_list2[i])
    fields_list2[i].grid(column=0, row=i)
    button_equal = Button(root, text="=", command=list_functions[i]).grid(column=3, row=i)
    value_result_list[i] = StringVar()
    list_entry_result[i] = Entry(root, width=10, textvariable=value_result_list[i])
    list_entry_result[i].grid(column=4, row=i)
