...
def highlight_board():
    for index in range(len(list_square)):
        row, column = index//(length-2), index%(length-2)
        color = list_board_colors[(row+column)%2]
        list_square[index].config(background=color)
...
def highlight_row():
    ...
    column_selected, row_selected = get_selected_address()
    for index in range(len(list_square)):
        column, row = index%(length-2), index//(length-2)
        square_selected = row == row_selected and column == column_selected
        if square_selected:
            list_square[index].config(background=color_hightlight_square)
